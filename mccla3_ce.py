...
class Cola(pl.LightningModule):
    ...
    def training_step(self, x, batch_idx):
        ...
        diag = torch.minimum(torch.minimum(diag1,diag2),diag3)

        y_hat = torch.maximum(torch.maximum(y_hat1,y_hat2),y_hat3)
        y_hat_new = torch.sub(y_hat,torch.diag_embed(torch.diag(y_hat)))
        y_hat_new = torch.add(y_hat_new,torch.diag_embed(diag))
        #max_sim = torch.log(torch.sum(y_hat,1))
        #diag = torch.diag(y_hat)
        #print(diag)
        #diff = torch.sub(max_sim,diag)
        #loss = diff.sum()
